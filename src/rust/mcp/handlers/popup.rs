...
fn find_ui_command() -> Result<String> {
    // 1. 优先尝试与当前 MCP 服务器同目录的续言设置命令
    if let Ok(current_exe) = std::env::current_exe() {
        if let Some(exe_dir) = current_exe.parent() {
            let local_ui_path = exe_dir.join("续言设置");
            ...
            #[cfg(windows)]
            {
                let local_ui_path = exe_dir.join("续言设置.exe");
                if local_ui_path.exists() && is_executable(&local_ui_path) {
                    return Ok(local_ui_path.to_string_lossy().to_string());
                }
            }
    ...
    // 2. 尝试全局命令（最常见的部署方式）
    if test_command_available("续言设置") {
        return Ok("续言设置".to_string());
    }
