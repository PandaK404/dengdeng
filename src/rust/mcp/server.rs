...
use super::types::{XuRequest, JiyiRequest};
...
impl ServerHandler for XuServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            instructions: Some("Xu 智能代码审查交互工具，支持交互式对话和记忆管理".to_string()),
        }
    }
    ...
    async fn list_tools(
        ...
        if let serde_json::Value::Object(schema_map) = xu_schema {
            tools.push(Tool {
                name: Cow::Borrowed("xu"),
    ...
    async fn call_tool(
        ...
        match request.name.as_ref() {
            "xu" => {
                ...
                let xu_request: XuRequest = serde_json::from_value(arguments_value)
                    .map_err(|e| McpError::invalid_params(format!("参数解析失败: {}", e), None))?;

                // 调用续言工具
                InteractionTool::xu(xu_request).await
            }
            "ji" => {
                // 检查记忆管理工具是否启用
                if !self.is_tool_enabled("ji") {
